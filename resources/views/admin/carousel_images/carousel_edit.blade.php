...
<head>
    ...
    <title>Admin Panel - Edit Carousel-Image</title>
...
<body>
    ...
    <main class="flex-grow-1 p-3" id="main-content">
        <h1 class="top-title" style="margin-top: 70px">Update Carousel</h1>

       <!-- Edit Form -->
       <form action="{{ route('admin.carousel.update', $carouselImage->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="form-box">
            <div class="image-upload" style="margin-top: 10px;">
                ...
                @if ($carouselImage->image_path)
                    <img src="{{ asset('storage/' . $carouselImage->image_path) }}" alt="Current Image" style="width: 100px; margin-top: 10px;">
                @endif
            </div>
    
            <div class="form-submit" style="gap: 20px">
                <button type="submit">Update</button>
                <a href="{{ route('carousel.carousel_view') }}" class="btn btn-info">Back</a>
            </div>
        </div>
    </form>
