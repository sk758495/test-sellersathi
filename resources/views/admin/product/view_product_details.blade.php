...
<head>
    ...
    <title>Admin Panel - Product Details Page</title>
...
<body>
    ...
    @include('admin.pages.navbar-sidebar')
        ...
        <main class="flex-grow-1 p-3" id="main-content">
            ...
            <!-- Add Brand Form -->
<div id="add-brand-form" action="" method="POST" enctype="multipart/form-data">

    <div class="form-box">
        <div class="form-group">
            <label for="product-name">Product Name</label>
            <input type="text" id="product-name" value="{{ $product->product_name }}" name="product_name" disabled>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            <label for="brand">Gujju Category</label>
            <input type="text" id="category" value="{{ $product->category ? $product->category->name : 'N/A' }}" disabled>
        </div>
        <div class="form-group">
            <label for="brand">Brand</label>
            <input type="text" id="brand" value="{{ $product->brand ? $product->brand->name : 'N/A' }}" disabled>
        </div>
        <div class="form-group">
            <label for="category">Category</label>
            <input type="text" id="category" value="{{ $product->brandCategory ? $product->brandCategory->name : 'N/A' }}" disabled>
        </div>
        <div class="form-group">
            <label for="subcategory">Subcategory</label>
            <input type="text" id="subcategory" value="{{ $product->subcategory ? $product->subcategory->name : 'N/A' }}" disabled>
        </div>


        <div class="form-group">
            <label for="quantity">Quantity</label>
            <input type="number" id="quantity" name="quantity" value="{{ $product->quantity }}" min="1" disabled>
        </div>
        <div class="form-group">
            <label for="color-name">Color Name</label>
            <input type="text" id="color-name" name="color_name" value="{{ $product->color_name }}" disabled>
        </div>
        <div class="form-group">
            <label for="color-code">Color Code</label>
            <input type="text" id="color-code" name="color_code" value="{{ $product->color_code }}" disabled>
        </div>
        <div class="form-group">
            <label for="price">RRP (Price)</label>
            <input type="number" id="price" name="price" value="{{ $product->price }}" step="0.01" disabled>
        </div>
        <div class="form-group">
            <label for="cost-price">Cost Price</label>
            <input type="number" id="cost-price" name="cost_price" value="{{ $product->cost_price }}" step="0.01" disabled>
        </div>
        <div class="form-group">
            <label for="discount-price">Discount Price</label>
            <input type="number" id="discount-price" name="discount_price" value="{{ $product->discount_price }}" disabled>
        </div>
        <div class="form-group">
            <label for="lead-time">Lead Time</label>
            <input type="text" id="lead-time" name="lead_time" value="{{ $product->lead_time }}" disabled>
        </div>

        <div class="image-upload">
            <label for="main-images-2">Product Main Image</label>
            <input type="file" id="main-images-2"  name="main_images" accept="image/*" disabled>
            <div><img src="{{ asset('storage/' . $product->main_image) }}" alt="{{ $product->product_name }}"></div>
            <div class="image-preview" id="main-image-preview-2"></div>
        </div>
<!-----
        <div class="image-upload">
            <label for="product-images">Product Images (up to 4 images)</label>
            <input type="file" id="product-images" name="product_images[]" accept="image/*" multiple disabled>
...
-->
    ...
    <!-- Submit Button -->
    <button class="btn btn-warning"><a href="{{ route('admin.products.edit_product', ['id' => $product->id]) }}" style="text-decoration: none; color: white">Edit Product</a></button>
    ...
    function confirmDelete(productId) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                // If the user confirmed, submit the delete form
