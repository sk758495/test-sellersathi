...
<head>
    ...
    <title>Admin Panel - Edit Brand Category</title>
...
<body>
    ...
    <h1 style="margin-top: 70px">Edit Brand Category for Sub - Category</h1>

    <form action="{{ route('pages.update_brand_category', $brandCategory->id) }}" method="POST" enctype="multipart/form-data">
        @csrf

        <div class="form-box">
            <div class="form-group">
            <label for="name">Brand Category Name</label>
            <input type="text" name="name" class="form-control" value="{{ old('name', $brandCategory->name) }}" required>
        </div>
        </div>
        <div class="form-box">
            <div class="form-group">
                <label for="image">Brand Category Image</label>
        ...
        <div class="form-submit" style="gap: 10px">
        <button type="submit">Update Brand Category</button>

        <a href="{{ route('pages.view_brands') }}" class="btn btn-secondary" style="height: 48px; align-content: center;">Cancel</a>
    </div>
    </form>
