    ...
    <!-- Body Part -->
        <main class="flex-grow-1 p-3" id="main-content">
            ...
            <div class="heading" style="display: flex; justify-content: space-between">
                ...
                <a href="{{ route('pages.add_brand') }}" class="btn btn-success" style="height: 30px; margin-top: 70px;">Add Brand</a>
            </div>
                            ...
                            <tbody>
                                @foreach($brands as $brand)
                                    <tr>
                                        ...
                                        <td class="text-truncate" style="max-width: 150px;">{{ $brand->name }}</td>
                                        <td>{{ $brand->created_at->format('Y-m-d') }}</td>
                                        <td>
                                            <a href="{{ route('pages.edit_brand', $brand->id) }}" class="btn btn-warning btn-sm">Edit</a>

                                            <form action="{{ route('pages.delete_brand', $brand->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to delete this brand?');">
                                                @csrf
                                                @method('DELETE')
            ...
            <!---Add Brand Category here-->
            <div class="card shadow-sm mb-4">
                <div class="card-header">
                    <h5 class="mb-0">Brand Category List</h5>
                </div>
                <div class="card-body">
                    <!-- Responsive and scrollable table -->
                    <div class="table-responsive" style="max-height: 400px;">
                                    ...
                                    <tr>
                                        ...
                                        <td><ul>
                                            @foreach ($brand->brandCategories as $brandCategory)
                                                <li>
                                                    {{ $brandCategory->name }} <a href="{{ route('pages.edit_brand_category', $brandCategory->id) }}" class="edit-icon" class="btn btn-info btn-sm"><i class="fas fa-edit"></i></a>

                                                    <form action="{{ route('pages.delete_brand_category', $brandCategory->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to delete this brand - Category?');">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="delete-icon" class="btn btn-danger btn-sm"><i class="fas fa-trash"></i></button>
                                                    </form>


                                                    @if ($brandCategory->images->isNotEmpty())
                                                            <!-- Display the first image in the images collection -->
                                                            <img src="{{ url('storage/' . $brandCategory->images->first()->image) }}" style="width: 30px;" class="card-img-top" alt="Category Image">
                                                        @else
                                                    ...
                                                    <ul>
                                                        @foreach ($brandCategory->subcategories as $subcategory)
                                                            <li>
                                                                {{ $subcategory->name }}
                                                                <form action="{{ route('pages.destroy_subcategory', $subcategory->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to delete {{ $subcategory->name }} -  SubCategory?');">
                                                                @csrf
                                                                @method('DELETE')
                                                    ...
                                                    <a href="{{ route('pages.add_subcategory', $brandCategory->id) }}" class="btn btn-warning btn-sm">Add Brand Sub - Category</a>
                                        ...
                                        <td>
                                            ...
                                            <a href="{{ route('pages.add_brand_category', $brand->id) }}" class="btn btn-warning btn-sm">Add Brand Category</a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
